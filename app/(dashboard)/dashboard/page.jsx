   ...
   const fetchBookings = useCallback(async () => {
    ...
    try {
        ...
        const res = await fetch(
            `${BASE_API}/bookings/me.php?user_id=${session.user.id}&email=${session.user.email}`,
            {
                cache: 'no-store',
            }
        );
    ...
    const stats = useMemo(() => {
        const active = bookings.filter((b) =>
            ['active', 'confirmed', 'upcoming'].includes(
        ...
        const utility = bookings.reduce((acc, booking) => {
            if (booking.car_name) {
                acc[booking.car_name] =
                    (acc[booking.car_name] || 0) + 1;
            }
    ...
    if (loading) {
        return (
            <CityDriveLoader message="syncing fleet data" />
        );
    }
    ...
    return (
        <div className="max-w-8xl mx-auto p-4 md:p-6 space-y-8 bg-[#F8F9FA] min-h-screen font-sans animate-in fade-in duration-700">
                ...
                <div className="flex items-center gap-3 bg-white p-2 rounded-2xl shadow-sm border border-gray-100">
                    <div className="pl-4 pr-2 text-right">
                        ...
                        <p className="text-xl font-black text-gray-900 leading-none">
                            K{stats.totalSpent.toLocaleString()}
                        </p>
                    </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <QuickStat
                    label="Utilized Models"
                    value={Object.keys(stats.utility).length}
                    icon={<FaCogs />}
                    trend="Fleet Variety"
                    color="bg-purple-500"
                />
                ...
                <section>
                    ...
                    <div className="bg-white rounded-[2rem] border border-gray-100 overflow-hidden shadow-sm">
                        {filteredHistory.length > 0 ? (
                            ...
                            <EmptyState message="No booking history found" />
                        )}
                    </div>
                </section>
            </div>
